...
var UI2KB={
    ...
    method: 'post',
    body: { }, // Javasc ript object payload.
    // json: true,
...
var penColourWS8_9_10_11 = "GREEN";
var penColourWS12_2_3 = "RED";
...
emitter.on('move pallet step 0', function movePallet0(neighbourStatusValues,WSN,CNV,productID) {
    ...
    if (neighbourStatusValues.length == 1) {
        if (WSN == 7) {
            ...
            emitter.emit('loading and unloading', neighbourStatusValues, WSN, CNV, productID, queryMovePalletLS);
        }
        else if(CNV!=3){
        ...
        emitter.once('loading and unloading', function loadUnload(neighbourStatusValues, WSN, CNV, productID, queryMovePalletLS) {

            var queryProductProcess = "query=PREFIX sn:<http://www.ontologies.com/G7_fastory.owl#> " +
                "SELECT ?products " +
                "WHERE {sn:master sn:hasOrdered ?products. ?products sn:hasPalletID ?ID. " +
                "FILTER (?ID = " + 123 + ")}";
            optionsKB.body = queryProductProcess;

            request(optionsKB, function (err, res, body) {
                var productUnderProcess = JSON.parse(body).results.bindings[0].products.value;
                 var queryunloading="query=PREFIX sn:<http://www.ontologies.com/G7_fastory.owl#> " +
                "SELECT ?needs " +
                "WHERE {<" + productUnderProcess + "> sn:hasNeed ?needs.}";
               // console.log("product: ", queryunloading);
                optionsKB.body=queryunloading;
                request(optionsKB,function(err,res,body){
                    console.log("counter here works");
                    var counter = JSON.parse(body).results.bindings[0];
                    console.log("counter here ",counter);
                    if( typeof(counter)=== typeof undefined){
                        console.log('unloading mf!!!!')

                        request.post('http://localhost:3000/RTU/SimROB1/services/UnloadPaper', function (err, res, body) {
                            setTimeout(function () {
                                request.post('http://localhost:3000/RTU/SimCNV1/services/TransZone35', function (err, res, body){});
                            },2500)
                        });
                    }
                        ...
                        request.post('http://localhost:3000/RTU/SimROB1/services/LoadPaper', function (err, res, body) {
                            ...
                            var resetValue = "update=PREFIX sn:<http://www.ontologies.com/G7_fastory.owl#> " +
                                "DELETE  " +
                                "WHERE {<" + productUnderProcess + ">  sn:hasPaperLoaded ?value}; " +
                                "INSERT {<" + productUnderProcess + ">  sn:hasPaperLoaded 1;} " +
                                "WHERE {}";
        ...
        optionsKB.body = queryHasPaperLoaded;
            ...
            request(optionsKB, function (err, res, body) {
                if (JSON.parse(body).results.bindings[0].paperLoaded.value == false) {
                    ...
                    emitter.emit('move pallet step 2',neighbourStatusValues,WSN,CNV,productUnderWork,queryMovePalletWS,productID)
                }
        ...
        request.post(JSON.parse(body).results.bindings[0].colour.value, function (err, res, body) {
            ...
            request(optionsKB, function (err, res, body) {
                ...
                request(optionsKB, function (err, res, body) {
                    ...
                    //console.log('product needs: ', productNeeds);
                    ...
                    if (productNeeds.length != 0) {
                       ...
                       for (var i = 0; i < productNeeds.length; i++) {
                           ...
                           //recipe.push(productNeeds[i].needs.value.split("_")[2]);
                           ...
                           recipe.push(productNeeds[i].needs.value.split("_")[2]);
                            ...
                            for (var i = 0; i < productNeeds.length; i++) {
                                if (robotPenColour == productNeeds[i].needs.value.split("_").pop().toUpperCase()) {
                                    //console.log(productNeeds[i].needs.value.split("_").pop().toUpperCase());
                                    request.post(transportURL1, function (err, res, body) {
                                        request.post('http://localhost:3000/RTU/SimCNV'+WSN+'/services/TransZone23',function(err,res,body){
                                            if(err){
                                                console.log(err)
                                            }
                                    ...
                                    setTimeout(function(){
                                        ...
                                        emitter.emit('perform drawing',WSN,CNV,queryMovePalletWS,recipe,productUnderWork,robotPenColour);
                                    },3500);
                                ...
                                // no match in colours found
                                    ...
                                    request.post(transportURL2, function (err, res, body) {
                                        console.log('works here no match');
                                        request.post('http://localhost:3000/RTU/SimCNV'+WSN+'/services/TransZone45',function(err,res,body){});
...
emitter.on('perform drawing',function drawPart(WSN,CNV,queryMovePalletWS,recipe,productUnderWork,robotPenColour) {
    console.log('recipe' ,recipe.length);


      for(var i=0;i<recipe.length;i++) {
        var deleteServices="update=PREFIX sn:<http://www.ontologies.com/G7_fastory.owl#> " +
            "DELETE " +
            "WHERE {<" + productUnderWork + "> sn:hasNeed sn:recipe_"+recipe[i]+"_"+robotPenColour.toLowerCase()+"}";
        InsertKB(deleteServices);
        console.log('deleting',deleteServices);
    }
       if(recipe.length==3) {
           console.log('comes here 3');
           var value1 = Number(recipe[0]), value2 = Number(recipe[1]), value3 = Number(recipe[2]);
           request.post('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value1,function (err,res,body) {});
           setTimeout(function(){   request.post('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value2,function (err,res,body) {}); },5000);
           setTimeout(function(){   request.post('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value3,function (err,res,body) {}); },10000);
           setTimeout(function(){   request.post('http://localhost:3000/RTU/SimCNV'+WSN+'/services/TransZone35',function (err,res,body) {}); },13000);
       }

       if(recipe.length==2) {
        console.log('comes here 2');
        var value1 = Number(recipe[0]), value2 = Number(recipe[1]);
        request.post('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value1,function (err,res,body) {});
        setTimeout(function(){   request.post('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value2,function (err,res,body) {}); },5000);
        setTimeout(function(){   request.post('http://localhost:3000/RTU/SimCNV'+WSN+'/services/TransZone35',function (err,res,body) {}); },9000);
    }
        if(recipe.length==1){
        console.log('comes here 1');
        var value1 = Number(recipe[0]);
        console.log("value 1 here",value1);
        console.log('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value1);
        request.post('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value1,function (err,res,body) {});
        setTimeout(function(){   request.post('http://localhost:3000/RTU/SimCNV'+WSN+'/services/TransZone35',function (err,res,body) {}); },5000);
       // setTimeout(function(){   request.post('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value2,function (err,res,body) {}); },4000);
     //   setTimeout(function(){   request.post('http://localhost:3000/RTU/SimROB'+WSN+'/services/Draw'+value3,function (err,res,body) {}); },8000);
        ...
        if (json.results.bindings.length == 2 && WSN != 7 && WSN != 1) {
            ...
            var interval2 = setInterval(function () {
                request.post(neighbour2, function (error, response, body) {
                    ...
                    console.log('neighbours: ',neighbourStatusValues);
                });
            },500);
        }
        else {
